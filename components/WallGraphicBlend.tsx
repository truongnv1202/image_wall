...
type Props = {
  ...
  /**
   * Bỏ lớp nền tối + vignette phía dưới watermark.
   * Bật khi nền là ảnh wallpaper — nếu không, lớp `bg-[#0b1020]/75` che gần hết ảnh.
   */
 ...
 * 3. Nền trong lớp (mờ trên lưới) — có thể tắt khi `hideBackdrop`.
 * 4. Ảnh watermark phủ toàn màn + `mix-blend-mode` và opacity (upload).
 */
export function WallGraphicBlend({ blendMode, overlayOpacity, hideBackdrop = false }: Props) {
  return (
    <div
      aria-hidden
